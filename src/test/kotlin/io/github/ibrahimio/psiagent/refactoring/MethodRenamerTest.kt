...
class MethodRenamerTest : BasePlatformTestCase() {
    ...
    fun testRenameMethodWithUsagesAcrossMethods() {
     ...
     fun testRenameJavaInterfaceSucceeds() {
        val javaCode = """
        ...
        val renamer = MethodRenamer(project)
        val result = renamer.renameSymbol(filePath, "Greeter", "FriendlyGreeter")

        assertTrue("Rename should succeed", result.success)
        assertNotNull("Renamed interface should exist", renamer.findSymbolInFile(psiFile, "FriendlyGreeter"))
        assertNull("Old interface name should no longer exist", renamer.findSymbolInFile(psiFile, "Greeter"))
    }

    fun testRenameJavaFieldSucceeds() {
        val javaCode = """
            public class Config {
        ...
        val psiFile = myFixture.configureByText("Config.java", javaCode)
        val filePath = psiFile.virtualFile.url

        val renamer = MethodRenamer(project)
        val result = renamer.renameSymbol(filePath, "apiKey", "secretKey")

        assertTrue("Rename field should succeed", result.success)
        assertNotNull("Renamed field should exist", renamer.findSymbolInFile(psiFile, "secretKey"))
        assertNull("Old field name should no longer exist", renamer.findSymbolInFile(psiFile, "apiKey"))
    }

    fun testFindsAllNodeTypesInFile() {
        val javaCode = """
        ...
        val psiFile = myFixture.configureByText("MultiNodeExample.java", javaCode)
        val renamer = MethodRenamer(project)

        // Should be able to find all these nodes
        assertNotNull("Should find class", renamer.findSymbolInFile(psiFile, "MultiNodeExample"))
        assertNotNull("Should find method", renamer.findSymbolInFile(psiFile, "process"))
        assertNotNull("Should find field", renamer.findSymbolInFile(psiFile, "config"))
    }
}
